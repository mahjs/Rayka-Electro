    ...
    <div className="flex flex-col w-[22.4375rem] justify-between rounded-xl border-2 border-[#ffffff18] gap-4 py-8 px-[1.28rem] ">
      ...
      <div className="flex flex-wrap gap-2 justify-start">
        {featuresList.map((feature, index) => (
          <div
            key={index}
            className="flex items-center rounded-3xl px-3 py-5 gap-2 bg-[#ffffff18] h-[2rem] backdrop-blur-[37px] text-white"
          >
      ...
      <div className="flex flex-col ">
        <p className="text-start text-white opacity-50 mt-auto body">قیمت/ماهانه</p>
        <div className="flex items-center gap-2">
          <p className="text-center text-white  mt-auto body">{price}</p>
          <img alt="edit" src={edit} className="w-[10px] h-[10px] cursor-pointer" />{' '}
        </div>
      </div>
    </div>
